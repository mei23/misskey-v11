...
export async function explain2<T extends ObjectLiteral>(qb: QueryBuilder<T>) {
	const r = await explain(qb, { analyze: true, verbose: false, buffers: false }, 'text') as { 'QUERY PLAN': string }[];
	const t = r.map(x => x['QUERY PLAN']).join('\n');
	//const t = JSON.stringify(r[0]?.['QUERY PLAN'], null, 2)
	return t;
}
